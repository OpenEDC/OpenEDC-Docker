...
class DateSettingsModal extends HTMLElement {
    setCurrentValues(currentValues) { this.currentValues = currentValues }
    setCallback(callback) { this.callback = callback }
    connectedCallback() {
        this.innerHTML = `
            <div class="modal is-active" id="date-settings-modal">
                <div class="modal-background"></div>
                <div class="modal-content is-medium is-fullheight-mobile">
                    <div class="box is-flex is-flex-direction-column" id="date-settings-modal-elements">
                        <h2 class="subtitle" i18n="date-settings-heading"></h2>
                        <label class="checkbox">
                            <input type="checkbox" id="checkbox-day">
                            <span i18n="day"></span>
                        </label>
                        <label class="checkbox">
                            <input type="checkbox" id="checkbox-month">
                            <span i18n="month"></span>
                        </label>
                        <label class="checkbox">
                            <input type="checkbox" id="checkbox-year">
                            <span i18n="year"></span>
                        </label>
                        <label class="checkbox">
                            <input type="checkbox" id="checkbox-optional">
        ...
        `;
        const keys = ['day', 'month', 'year', 'optional'];
        this.currentDateSettings = {}
        if(this.currentValues) {
            this.currentValues.split(";").forEach(value => this.currentDateSettings[value] = true);
            keys.forEach(key => {
                if(!this.currentDateSettings[key]) this.currentDateSettings[key] = false;
            })
        }
        else keys.forEach(key => this.currentDateSettings[key] = true);
        Object.entries(this.currentDateSettings).forEach(([key, value]) => {
            this.querySelector(`#checkbox-${key}`).checked = value;
        })

        this.querySelectorAll('input').forEach(checkbox => checkbox.onchange = () => {
            this.currentDateSettings[checkbox.getAttribute('id').substring(checkbox.getAttribute('id').indexOf('-') + 1)] = checkbox.checked;
        })
        this.querySelector(".modal-background").onclick = () => {
            this.save();
            this.remove();
    ...
    save() {
        let allTrue = true;
        const settingsStringsArray = [];
        Object.entries(this.currentDateSettings).forEach(([key, value]) => {
            if(!value) allTrue = false;
            if(value) settingsStringsArray.push(key)
        })
        if(allTrue) {
            this.callback("");
            return;
        }
        this.callback(settingsStringsArray.join(";"));
    }
}
...
window.customElements.define("date-settings-modal", DateSettingsModal);