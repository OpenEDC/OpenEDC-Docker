...
import { OpenEDCNotification, OpenEDCNotificationAction } from "./helper/notificationhelper.js";
...
ioHelper.addGlobalEventListener("DOMContentLoaded", async () => {
    ...
    const enableServiceWorker = !developmentOrigins.some(origin => window.location.origin.includes(origin) || window.location.href.includes(origin));
    ...
    versionCheckInterval = setInterval(async () => {
        ...
        if(newVersion) {
            ...
            if(notifications.length == 0) {
                 let notification = new OpenEDCNotification(
                    ...
                    [new OpenEDCNotificationAction('update', 'updateToNewVersion', 'button')],
                    'fa-wrench', null); 
                /* let notification = {id: 'test1234', creationDate: new Date().toISOString(), creator: "System",  title: languageHelper.getTranslation('new-version'), message: languageHelper.getTranslation('new-version-notification'), identifier: 'new-version-info', isSystem: true,
                   actions: [{name:'update', callback: 'updateToNewVersion', type: 'button'}], icon:
                    'fa-wrench', expirationDate: null, status: 'new'}; */
...
window.showNotifications = async(e) => {
    ...
    if($('#notification-div')) {
        ...
        div.classList = 'container___menu'
