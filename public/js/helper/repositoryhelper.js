...
export const getModels = async urlParams => {
    ...
    if(tokenParameters[0]) sessionStorage.setItem("mdmUserToken", tokenParameters[0]);
    
    sessionStorage.removeItem("predecessorModelId");
    if(modelParameters[0]) sessionStorage.setItem("predecessorModelId", modelParameters[0]);
